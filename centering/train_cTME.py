...
def m_estimator_center(X):
    N, D = X.shape
    X_aug = np.hstack((np.ones((N, 1)), X))
    cov1 = m_estimator(X_aug)
    cov1 /= cov1[0, 0]
    center = cov1[1:D+1, 0]
    cov = cov1[1:D+1, 1:D+1] - np.outer(center, center)
...
def main():
    ...
    V = np.dot(W,W.T)
    W_hat = np.real(sqrtm(V))
    W_hat = W_hat + 1e-9 * np.eye(W_hat.shape[0])
    #U,S,Vh = np.linalg.svd(W_hat,full_matrices=True)
    #print("Singular value of W_hat:",S)
    
    Cov = m_estimator_center(W_hat)
