...
def get_datasets(args):
    if args.datasets == 'MNIST':
        ...
        data_dir = os.path.join(os.path.dirname(__file__), 'data/tiny-imagenet-200')
        ...
        if args.corrupt > 0:
            path = 'tiny_imagenet_' + str(args.corrupt) + '_corruptedlabel'
            print('Corrupt:', args.corrupt)
            print('Path:', path)
            ...
            # Apply label corruption
                ...
                num_corrupt = int(len(train_dataset) * args.corrupt)
                corrupt_indices = np.random.choice(len(train_dataset), num_corrupt, replace=False)

                for idx in corrupt_indices:
                    _, label = train_dataset.samples[idx]
                    corrupt_label = np.random.choice(num_classes)
                    while corrupt_label == label:  # Ensure corruption changes the label
                        corrupt_label = np.random.choice(num_classes)
                    train_dataset.samples[idx] = (train_dataset.samples[idx][0], corrupt_label)

                torch.save(train_dataset, path)
                print(f"Saved corrupted dataset to {path}")
